...
def full_Ns_MM(file_list: list[str],) -> list[dict[int, dict[str, list[int]]]]:
...
def get_Ns_MM(file_list: list[str]) -> list[dict[int, dict[str, int]]]:
    ...
    for file_name in file_list:
        if file_name.isalnum():
            print(f"{file_name=} is continuing")
            continue
        for idx, char in enumerate(file_name):
            if char.isalpha():
                stop_at = idx
...
if __name__ == "__main__":
    current_directory = Path(f"./{sys.argv[1]}")
    files = find_files_to_analyse(current_directory)
    print(files)
    print("getting the ns")
